...
void main() {
  group('LineNode Congestion and Load Tests', () {
    ...
    test('Boundary conditions for congestion color rules', () {
      ...
      expect(node0.crowdColor, equals(const Color(0xff18ad3e)), reason: '0 load must be green');

      const node59 = LineNode(id: '59', name: 'N', region: 'R', load: 59);
      expect(node59.crowdColor, equals(const Color(0xff18ad3e)), reason: '59 load must be green');

      // Rule 2: Yellow 60 <= yellow <= 85
      const node60 = LineNode(id: '60', name: 'N', region: 'R', load: 60);
      expect(node60.crowdColor, equals(const Color(0xffff9822)), reason: '60 load must be yellow');

      const node72 = LineNode(id: '72', name: 'N', region: 'R', load: 72);
      expect(node72.crowdColor, equals(const Color(0xffff9822)), reason: '72 load must be yellow');

      const node85 = LineNode(id: '85', name: 'N', region: 'R', load: 85);
      expect(node85.crowdColor, equals(const Color(0xffff9822)), reason: '85 load must be yellow');

      // Rule 3: Red > 85
      const node86 = LineNode(id: '86', name: 'N', region: 'R', load: 86);
      expect(node86.crowdColor, equals(const Color(0xffff2d2d)), reason: '86 load must be red');

      const node100 = LineNode(id: '100', name: 'N', region: 'R', load: 100);
      expect(node100.crowdColor, equals(const Color(0xffff2d2d)), reason: '100 load must be red');
    });
    ...
    test('Host and port getters parse raw vless URI correctly', () {
      const node = LineNode(
        ...
        name: 'HK Vless',
        region: 'HK',
        raw: 'vless://uuid-1234@hk01.example.com:443?encryption=none&security=tls#HK-01',
      );
    ...
    test('Host and port fallback gracefully for invalid or empty raw URI', () {
      const node = LineNode(id: '1', name: 'Invalid', region: 'HK', raw: 'not-a-valid-uri');
